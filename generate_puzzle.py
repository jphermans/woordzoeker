#!/usr/bin/env python3
"""
Dagelijkse Woordzoeker Generator
"""
...
WORD_LISTS = [
    ["APPEL", "BOOM", "HUIS", "WATER", "VOGEL", "FIETS", "BOEK", "TAART", "KOEK", "ZON", "MAN", "VROUW"],
    ["KONING", "KASTEEL", "RIDDER", "DRAAK", "PRINS", "SPELER", "SPEL", "MUNT", "GOUD", "HELD", "ZWAARD", "SCHILD"],
    ["KOFFIE", "THEE", "MELK", "SAP", "BOTER", "KAAS", "BROOD", "EI", "FRUIT", "GROENTE", "VLEES", "VIS"],
    ["STRAND", "ZEE", "DUIN", "SCHELP", "KRAB", "VIS", "WIND", "SURF", "BOOT", "WOLK", "REGEN", "STER"],
    ["DIER", "KAT", "HOND", "PAARD", "SCHAAP", "VARKEN", "KIP", "EEND", "MUIS", "KOE", "ESEL", "LEEUW"],
    ["SCHOOL", "BOEK", "PEN", "LOODS", "LINIAAL", "LIJM", "SCHAAR", "PAPIER", "TAFEL", "STOEL", "KLAS", "LES"],
    ["STAD", "DORP", "STRAAT", "WEG", "BRUG", "PARK", "TUIN", "BOS", "BERG", "RIVIER", "MEER", "EILAND"],
    ["ROOD", "BLAUW", "GROEN", "GEEL", "PAARS", "ORANJE", "WIT", "ZWART", "BRUIN", "ROZE", "GRIJS", "GOUD"]
]
...
def generate_puzzle():
    ...
    seed = int(hash(today) % (10 ** 8))
    random.seed(seed)
    word_list = random.choice(WORD_LISTS)
    words = random.sample(word_list, 10)
    return words, seed, today
...
<body>
    <div class="container">
        <h1>🔍 Woordzoeker</h1>
        <p class="subtitle">Vind alle Nederlandse woorden! <span class="date-badge">{date_str}</span></p>
    ...
    <script>
        ...
        function handleMouseDown(e) {{ if (e.target.classList.contains('found')) return; isSelecting = true; startCell = {{row: parseInt(e.target.dataset.row), col: parseInt(e.target.dataset.col)}}; selectedCells = [startCell]; updateSelection(); }}
        function handleMouseEnter(e) {{ if (!isSelecting || e.target.classList.contains('found')) return; const currentCell = {{row: parseInt(e.target.dataset.row), col: parseInt(e.target.dataset.col)}}; updateSelectedPath(currentCell); }}
        function handleMouseUp() {{ if (!isSelecting) return; isSelecting = false; checkSelection(); }}
        function handleTouchStart(e) {{ e.preventDefault(); const touch = e.touches[0]; const cell = document.elementFromPoint(touch.clientX, touch.clientY); if (cell && cell.classList.contains('cell') && !cell.classList.contains('found')) {{ isSelecting = true; startCell = {{row: parseInt(cell.dataset.row), col: parseInt(cell.dataset.col)}}; selectedCells = [startCell]; updateSelection(); }} }}
        ...
        function checkWin() {{ if (foundWords.length === words.length) {{ const message = document.getElementById('message'); message.textContent = '🎉 Gefeliciteerd! Je hebt alle woorden gevonden!'; message.className = 'message success'; }} }}
...
if __name__ == "__main__":
    words, seed, date_str = generate_puzzle()
    ...
    with open('/a0/usr/projects/task_script/woordzoeker/index.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"Nieuwe woordzoeker gegenereerd voor {date_str}")
